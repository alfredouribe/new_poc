...
import { faceClass } from './libvbar-drv-dxface.so'
 ...
 * @returns true/false
 */
face.init = function (options, capturerRgbId, capturerNirId, dbPath, score = 0.6, fflitMax = 5000, mapPath = "/etc/.cameraCalibration", saveFacePath, saveFaceThumbnailPath) {
...
face.addFaceFeatures = function (userId, options) {
	...
	return faceObj.addFaceFeatures(userId, options.feature)
}
/**
 * 保存注册快照
 * @returns true/false
 */
face.regSnapshot = function () {
	return faceObj.regSnapshot()
}
/**
 * 更新配置
 ...
 */
face.getIspBrightness = function () {
	return faceObj.getIspBrightness()
}
/**
 * 查询用户总数
 * @returns number
